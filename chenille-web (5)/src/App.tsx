...
const PROJECTS: Project[] = [
  {
    title: "复合基底SAW器件温稳探索",
    background: "核心成员。使用PEST分析行业趋势，调研射频器件市场，并构建竞品分析体系。",
    role: "核心成员",
    tasks: [
      "使用PEST模型分析射频器件行业趋势",
      "深度调研射频器件市场竞争格局",
    ...
    projects: [
      {
        ...
        responsibilities: [
          ...
          "推动系统支持多格式文件的多人分层批注功能，实现高效协作闭环。"
        ],
        outcomes: [
          "通过功能优先级规划，使整体研发周期缩短约 20%。",
          "内测阶段获得 30+ 合作方反馈，核心功能满意度达到 92%。"
        ]
      },
      {
...
const CAMPUS_EXPERIENCE = [
  { 
    ...
    org: "天津科技大学人工智能学院学生会",
    details: [
      "活动统筹：负责学院大型文体活动策划与执行，统筹300人规模迎新晚会、文娱大会等活动，协调跨部门资源推进方案制定、物料准备与现场执行。",
      "流程优化：优化部门工作流程与责任制度，建立任务分工与信息共享机制，提高团队协作效率。",
      "活动主持：担任重要活动主持人，负责流程设计与现场节奏把控，确保活动顺利进行。"
    ]
  },
  { 
    role: "播音主持部｜副部长", 
    org: "天津科技大学人工智能学院融媒体中心",
    details: [
      "团队管理：负责部门日常运营与成员管理，制定并实施主持与播音培训计划，提升团队专业能力。",
      "活动策划：组织迎新活动、朗读者比赛等校园品牌活动，协调资源推进活动完整落地。"
    ]
  },
  { 
    role: "讲解员 & 培训小组长", 
    org: "天津科技大学人工智能实践基地",
    details: [
      "讲解接待：每年承担30余场政企接待讲解任务，向政府、企业及高校专家介绍人工智能技术应用与基地成果。",
      "团队培训：负责新成员培训与讲解技巧指导，建立系统培训流程，并带教多批新成员。",
      "获得“金牌讲解员”称号。"
    ]
  },
  { 
    ...
    org: "天津科技大学",
    details: [
      "信息管理：负责班级信息通知与反馈收集，保障师生沟通顺畅。",
      "内容创作：负责活动摄影、视频剪辑与活动总结撰写，累计完成10余篇活动记录。",
      "平台运营：创建并运营班级公众号，负责内容策划、编辑与发布。"
    ]
  }
];
          ...
          <div className="relative group">
            ...
            <div className="relative w-full aspect-square brutal-card bg-brand-blue overflow-hidden brutal-card-hover z-10">
              <img 
                ...
                onError={(e) => {
                  // Fallback to a high-quality anime illustration if local file is missing/empty
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="brutal-card p-8 bg-accent-blue text-white brutal-card-hover flex flex-col relative overflow-hidden">
            ...
            <div className="mt-4 pt-4 border-t border-white/20 flex justify-center">
              <IllustrationRoadmap className="text-white opacity-40 animate-pulse" />
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section id="experience" pattern={true}>
        ...
        <div className="relative border-l-8 border-black ml-4 pl-12 space-y-12">
          ...
          >
            <div className="w-8 h-8 bg-black rounded-full absolute left-[-56px] top-6 border-4 border-white z-10 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"></div>
            <div className="brutal-card p-8 bg-white w-full brutal-card-hover group relative overflow-hidden">
              <h3 className="text-3xl font-black mb-6">在校经历</h3>
              <div className="grid gap-6">
                {CAMPUS_EXPERIENCE.map((item, i) => (
                  <div key={i} className="flex flex-col md:flex-row gap-6 p-6 border-2 border-black rounded-xl bg-brand-pink/5 relative">
                    <div className="flex gap-4 items-center shrink-0 md:w-1/3">
                      <div className="w-12 h-12 bg-brand-blue border-2 border-black rounded-lg flex items-center justify-center shrink-0">
                        <Sparkles size={24} />
                      </div>
                      <div>
                        <div className="font-black text-lg">{item.role}</div>
                        <div className="text-sm text-black/50 font-bold">{item.org}</div>
                      </div>
                    </div>
                    <div className="flex-grow space-y-2">
                      {item.details.map((detail, di) => (
                        <p key={di} className="text-sm text-black/70 font-medium leading-relaxed">• {detail}</p>
                      ))}
                    </div>
                  </div>
                ))}
        ...
        <div className="grid md:grid-cols-2 gap-10">
          {PROJECTS.map((project, idx) => (
            <motion.div 
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="brutal-card p-8 bg-white flex flex-col brutal-card-hover group"
            >
              <div className="flex justify-between items-start mb-8">
                <div className="p-4 bg-brand-blue border-4 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] group-hover:bg-accent-blue group-hover:text-white transition-colors">
                  {project.type === 'internship' ? <Briefcase size={28} /> : <FolderKanban size={28} />}
                </div>
                <div className="flex flex-wrap gap-2 justify-end">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-[10px] font-black uppercase tracking-tighter border-2 border-black px-3 py-1 rounded-lg bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <h3 className="text-3xl font-black mb-6 group-hover:text-accent-blue transition-colors">{project.title}</h3>
              
              <div className="space-y-8 flex-grow">
                <div className="p-4 bg-black/5 rounded-xl border-2 border-dashed border-black/20 group-hover:border-accent-blue/30 transition-colors">
                  <h4 className="font-black text-xs uppercase text-black/40 mb-2">项目背景</h4>
                  <p className="text-sm text-black/70 leading-relaxed font-medium">{project.background}</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-brand-pink border-4 border-black flex items-center justify-center font-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">我</div>
                  <div>
                    <h4 className="font-black text-xs uppercase text-black/40">我的角色</h4>
                    <p className="text-base font-black">{project.role}</p>
                  </div>
                </div>
                <div>
                  <h4 className="font-black text-xs uppercase text-black/40 mb-3">主要工作</h4>
              ...
              <div className="mt-8 pt-6 border-t-2 border-black/5 flex justify-end">
                <button className="flex items-center gap-2 font-black text-sm hover:text-accent-blue transition-colors">
            ...
            <div className="space-y-8 relative z-10">
              ...
              <div className="flex gap-6">
                <div className="flex-1 flex items-center justify-center gap-3 p-6 bg-white border-4 border-black rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                  <span className="font-black">城市：天津</span>
                </div>
              </div>
            </div>
